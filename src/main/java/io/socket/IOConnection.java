...
class IOConnection implements IOCallback {
	...
	public void emit(SocketIO socket, String event, IOAcknowledge ack, Object... args) {
		try {
			...
			json.put("name", event);
			json.put("args", new JSONArray(Arrays.asList(args)));
				...
				if (data.length == 2) {
					try {
						...
						if (ack == null)
							...
							JSONArray array = JSON.parseArray(data[1]);
